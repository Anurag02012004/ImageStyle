# 🖼️ Example Images for Style Transfer

## What Types of Images to Upload?

### Content Image (What you want to stylize)
- **Best choices:**
  - Clear photographs with good detail
  - Portraits or faces
  - Landscapes or scenery
  - Objects or architecture
  - High-resolution images (but not too large - max 10MB)

- **Avoid:**
  - Already heavily stylized images
  - Very blurry or low-quality images
  - Images with too much text

### Style Image (The artistic style to apply)
- **Best choices:**
  - Famous paintings (Van Gogh, Picasso, Monet, etc.)
  - Artistic textures and patterns
  - Abstract art
  - Watercolor paintings
  - Oil paintings
  - Sketch or drawing styles

## 🎨 Free Example Images You Can Use

### Content Images (Test Images)
1. **Portrait/Person:**
   - https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=800
   - https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=800

2. **Landscape:**
   - https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800
   - https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=800

3. **Architecture:**
   - https://images.unsplash.com/photo-1511818966892-d7d671e672a2?w=800

4. **Nature:**
   - https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=800

### Style Images (Artistic Styles)

1. **The Starry Night (Van Gogh) - Classic Style:**
   - https://upload.wikimedia.org/wikipedia/commons/thumb/e/ea/Van_Gogh_-_Starry_Night_-_Google_Art_Project.jpg/800px-Van_Gogh_-_Starry_Night_-_Google_Art_Project.jpg

2. **The Great Wave (Hokusai) - Japanese Art:**
   - https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/Tsunami_by_hokusai_19th_century.jpg/800px-Tsunami_by_hokusai_19th_century.jpg

3. **Abstract Art:**
   - https://images.unsplash.com/photo-1557672172-298e090bd0f1?w=800

4. **Watercolor Style:**
   - https://images.unsplash.com/photo-1541961017774-22349e4a1262?w=800

## 🚀 Quick Test Combinations

### Test 1: Portrait with Starry Night Style
- **Content:** Portrait photo
- **Style:** The Starry Night painting
- **Expected:** Portrait with swirling, colorful brushstrokes

### Test 2: Landscape with Wave Style
- **Content:** Mountain/landscape photo
- **Style:** The Great Wave painting
- **Expected:** Landscape with wave-like patterns

### Test 3: Architecture with Abstract Art
- **Content:** Building photo
- **Style:** Abstract colorful art
- **Expected:** Building with abstract color patterns

## 💡 Tips for Best Results

1. **Image Size:** 
   - Good: 500x500 to 2000x2000 pixels
   - The app automatically resizes larger images to 1024px max

2. **File Format:**
   - Supported: PNG, JPG, JPEG, WEBP
   - Best quality: PNG
   - Smaller file: JPG

3. **Image Quality:**
   - Use clear, high-quality images
   - Avoid compressed or pixelated images

4. **Style Images:**
   - Bold, distinctive artistic styles work best
   - High contrast style images produce more dramatic results
   - Famous paintings often work very well

## 📥 How to Use These Images

1. **Right-click on any image link above**
2. **Select "Save Image As..." or "Download Image"**
3. **Save to your computer**
4. **Upload in the Style Transfer app**

## 🔗 Alternative Sources for Images

- **Unsplash** (free photos): https://unsplash.com
- **Pexels** (free photos): https://pexels.com
- **Wikimedia Commons** (public domain art): https://commons.wikimedia.org
- **Artvee** (public domain art): https://artvee.com

## ⚡ Quick Start Test

Try this quick test:
1. Download a portrait photo from Unsplash
2. Download "The Starry Night" painting image
3. Upload portrait as Content Image
4. Upload Starry Night as Style Image
5. Click "Transfer Style"
6. Enjoy your artistic creation! 🎨

